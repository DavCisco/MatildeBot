...
def ResetConfigFile(filename, newfile):
    ...
    try:
        ...
        if newfile:
            ...
            logger.warning('Configuration file reset in the current folder.')
        logger.warning(
...
def ChannelReport():
    ...
    try:
        connection = pymysql.connect(host=DBhost, port=int(
            DBport), user=DBuser, password=DBpass, db=dbName)
    except Exception as e:
    ...
    for channel in channels:
        ...
        if not filename:
            msg = 'No trials have been requested for channel ' + str(channel)
        else:
